...
def _model_allowed_for_key(request: Request, model: str):
    ...
    if not key_row:
        return None
    routes = _key_model_routes(request)
    # A route name is a synthetic identifier exposed only to this key.
    if model in routes:
        return None
    # Candidate models are intentionally separate from ``allowed_models``.
    # The router's private cloned request is the sole exception: it has
    # already resolved a configured route and must be able to dispatch that
    # exact backing model without making it directly callable by the client.
    internal_candidate = getattr(request.state, "model_route_candidate", "")
    if (
        getattr(request.state, "model_route_active", False)
        and model == internal_candidate
        and any(model in candidates for candidates in routes.values())
    ):
        return None
    raw = (key_row.get("allowed_models") or "").strip()
    allowed = {m.strip() for m in raw.split(",") if m.strip()}
    routed_candidates = {candidate for candidates in routes.values() for candidate in candidates}
    if model in routed_candidates:
        return (
            f"Model '{model}' is reserved as a fallback for this API key's model route. "
            "Call the route name instead."
        )
    if model in allowed:
        return None
    if not raw:
...
def _clone_request_for_model_route(request: Request, payload: dict,
    ...
    routed.state.model_route_candidate = payload.get("model", "")
    return routed
...
async def list_models(request: Request):
    ...
    catalog_fingerprint = {
        # Bump this whenever the catalog projection changes. Without it, a
        # short-lived Redis entry generated by an older release can expose a
        # route candidate until its TTL expires after a deploy.
